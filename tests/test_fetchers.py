...
class DeepSeekFetcherTests(unittest.TestCase):
    def _fetch(self, payload):
        from quota_providers import deepseek

        with mock.patch.object(deepseek, "_load_key", return_value="sk-test"), \
             mock.patch.object(deepseek.urllib.request, "urlopen", _urlopen_returning(payload)):
            return deepseek.fetch_deepseek_quota()

    def test_no_credentials(self):
        from quota_providers import deepseek

        with mock.patch.object(deepseek, "_load_key", return_value=None):
            res = deepseek.fetch_deepseek_quota()
        self.assertEqual(res.unavailable_reason, "no-credentials")

    def test_usd_balance_is_honest_card_without_windows(self):
        payload = {
            "is_available": True,
            "balance_infos": [
                {
                    ...
                    "topped_up_balance": "7.47",
                }
            ],
        }
        res = self._fetch(payload)
        self.assertIsNone(res.unavailable_reason)
        self.assertIsNone(res.plan)        # prepaid account — no plan concept
        self.assertEqual(res.windows, [])  # no percentage — balance has no denominator
        self.assertEqual(res.money, {"currency": "USD", "total": 8.47})
        self.assertNotIn("sk-test", repr(res))
        joined = "\n".join(res.details)
        self.assertIn("Solde total: 8.47 USD", joined)
        self.assertIn("Crédit offert: 1.00 USD", joined)
        self.assertIn("Crédit rechargé: 7.47 USD", joined)
        self.assertIn("État: disponible", joined)

    def test_cny_currency_is_respected(self):
        payload = {
            "is_available": True,
            "balance_infos": [{"currency": "CNY", "total_balance": "110.00"}],
        }
        res = self._fetch(payload)
        self.assertIn("110.00 CNY", "\n".join(res.details))

    def test_multiple_currencies_are_all_listed_and_usd_is_primary(self):
        payload = {
            "is_available": True,
            "balance_infos": [
        ...
        res = self._fetch(payload)
        joined = "\n".join(res.details)
        self.assertIn("110.00 CNY", joined)
        self.assertIn("8.47 USD", joined)
        self.assertEqual(res.money, {"currency": "USD", "total": 8.47})

    def test_timeout_is_explicit_and_fail_open(self):
        from quota_providers import deepseek
        ...
        with mock.patch.object(deepseek, "_load_key", return_value="sk-test"), \
             mock.patch.object(deepseek.urllib.request, "urlopen", _opener):
            res = deepseek.fetch_deepseek_quota()
        self.assertEqual(res.unavailable_reason, "timeout")

    def test_non_finite_balance_is_never_serialized_as_money(self):
        payload = {
            "is_available": True,
        ...
        self.assertIn("Solde total: — USD", "\n".join(res.details))

    def test_depleted_balance_reports_honest_status(self):
        payload = {
            "is_available": False,
            "balance_infos": [{"currency": "USD", "total_balance": "0.00"}],
        }
        res = self._fetch(payload)
        self.assertIsNone(res.unavailable_reason)  # still a configured card
        self.assertEqual(res.money, {"currency": "USD", "total": 0.0})
        self.assertIn("État: épuisé", "\n".join(res.details))

    def test_http_401_and_403_are_auth_failed(self):
        import urllib.error
            ...
            with self.subTest(status=status):
                def _opener(_req, timeout=None):  # noqa: ANN001, ARG001
                    raise urllib.error.HTTPError(
                        "url", status, "unauthorized", {}, BytesIO(b"")
                    )
    ...
    def test_http_429_reports_code(self):
        import urllib.error

        from quota_providers import deepseek

        def _opener(_req, timeout=None):  # noqa: ANN001, ARG001
            raise urllib.error.HTTPError("url", 429, "throttled", {}, BytesIO(b""))

        with mock.patch.object(deepseek, "_load_key", return_value="sk-test"), \
             mock.patch.object(deepseek.urllib.request, "urlopen", _opener):
            res = deepseek.fetch_deepseek_quota()
        self.assertEqual(res.unavailable_reason, "http-429")
