"""Quota plugin — per-provider quota status via /quota command and CLI.

Registers:
  * a ``/quota`` slash command for on-demand detail (triggers a refresh when
    the cache is stale), plus a ``hermes quota`` CLI command;
  * optional ``footer`` / ``usage_extra`` lifecycle hooks (only when the
    running Hermes build exposes them in ``VALID_HOOKS``) that append the
    same quota block to the runtime footer and /usage output.

The quota subsystem (fetchers + cache) lives entirely inside this plugin, so it
survives ``hermes update``. The Desktop widget is shipped separately under
``desktop/plugin.js`` and reaches the backend through the local gateway only.
"""
